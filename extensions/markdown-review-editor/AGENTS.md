...
- **Answer processing**:
  - AI returns answers in `<x-answer href="annotation-id">answer text</x-answer>` tags
  - `processAnswers()` extracts answers and removes x-answer tags from markdown
  - `applyAnswersToMarkdown()` updates annotations based on answers:
    - **Answered annotations**: User's comment wrapped in `<x-by author="User">`, answer wrapped in `<x-by author="AI">`, annotation kept
    - **Prompted but non-answered annotations**: Completely unwrapped (tags removed, content preserved)
    - **Discussion-only annotations** (no new reply): Preserved as-is (not sent to AI, not unwrapped)
  - Discussion threads grow with each Q&A cycle, preserving prior x-by elements
