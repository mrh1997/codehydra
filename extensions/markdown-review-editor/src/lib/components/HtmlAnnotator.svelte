<script lang="ts">
	...
	let annotations = $state<Map<string, AnnotationState>>(new Map());
	let editorHeights = $state<Map<string, number>>(new Map()); // Track actual CommentEditor heights
	...
	/**
	 * Get the editable content from messages array.
	 * The editable content is the last User message (or empty if none).
	 */
	function getEditableContent(messages: Message[]): string {
		if (messages.length === 0) return '';
		const lastMsg = messages[messages.length - 1];
		// Only return content if the last message is from User
		return lastMsg.author === 'User' ? lastMsg.content : '';
	}

	/**
	 * Get discussion messages (all messages except the last User message which is editable).
	 */
	function getDiscussionMessages(messages: Message[]): Message[] {
		if (messages.length === 0) return [];
		const lastMsg = messages[messages.length - 1];
				...
				mutation.removedNodes.forEach((node) => {
					if (node instanceof HTMLElement && node.tagName === 'X-ANNOTATION') {
						...
						if (id && annotations.has(id)) {
							...
							annotations = new Map(annotations);
							editorHeights = new Map(editorHeights);
						}
					}
				});
			}
	...
	$effect(() => {
		...
		const heightsSnapshot = [...editorHeights.entries()];
